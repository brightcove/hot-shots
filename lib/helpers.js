...
function overrideTagsToString(parent, child, telegraf, separator) {
  const tags = overrideTags(parent, child, telegraf);
...
function overrideTags2 (globalTags, globalTagsFullMemo, child, telegraf, separator) {
  ...
  for (const c of Object.keys(child)) {
    ...
    if (result === '') {
      result += serializedTagWithValue;
    }
    else {
      result += separator + serializedTagWithValue;
    }
  }
  if (usedGlobalKeys.size === 0) {
    result += separator + globalTagsFullMemo;
  }
  else {
...
module.exports = {
  ...
  overrideTagsToString: overrideTagsToString,
