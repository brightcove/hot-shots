...
function overrideTags (parent, child, telegraf) {
  ...
  // Handle child tags more efficiently by processing object vs array separately
  if (Array.isArray(child)) {
    // Child is already an array - process directly without conversion
    child.forEach(tag => {
    ...
    // Child is an object - process directly without string conversion
    Object.keys(child).forEach(key => {
      const sanitizedKey = sanitizeTags(key, telegraf);
      const sanitizedValue = sanitizeTags(child[key], telegraf);
      childCopy[sanitizedKey] = childCopy[sanitizedKey] || [];
      childCopy[sanitizedKey].push(sanitizedValue);
    });
  }
