...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
   ...
   */
  this.globalTagsArray  = typeof options.globalTags === 'object' ?
  ...
  if (availableDDEnvs.length > 0) {
    this.globalTagsArray = this.globalTagsArray.
  ...
  this.globalTagsMemo = helpers.overrideTagsToStringUnoptimized(this.globalTagsArray, [], this.telegraf, this.tagSeparator);
...
Client.prototype.send = function (message, tags, callback) {
  const tagsString = getTagString(this.globalTagsArray, this.globalTagsObjectSanitized, this.globalTagsMemo, tags, this.telegraf, this.tagSeparator);
...
function getTagString(globalTagsArray, globalTagsObject, globalTagsMemo, tags, telegraf, tagSeparator) {
  if (!(tags && typeof tags === 'object')) {
  ...
  return helpers.overrideTagsToStringUnoptimized(globalTagsArray, tags, telegraf, tagSeparator);
}
...
const ChildClient = function (parent, options) {
  ...
  Client.call(this, {
    ...
    globalTags  : typeof options.globalTags === 'object' ?
      helpers.overrideTagsUnoptimized(parent.globalTagsArray, options.globalTags, parent.telegraf) : parent.globalTagsArray,
 ...
 *   @option globalTagsArray {Array=} Optional tags that will be added to every metric
 */
