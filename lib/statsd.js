...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  if (this.socket && !this.isChild && options.protocol === PROTOCOL.UDS && this.udsGracefulErrorHandling) {
    const lastSocketCreateTime = Date.now();
    addUDSErrorHandler(this, lastSocketCreateTime);
  }
...
Client.prototype.sendMessage = function (message, callback) {
  ...
  if (socketWasMissing && this.protocol == PROTOCOL.UDS) {
    trySetNewSocket(this);
    if (this.socket && !this.isChild && this.udsGracefulErrorHandling) {
      // On success, add custom UDS error handling.
      addUDSErrorHandler(this, Date.now());
    }
  }
...
function addUDSErrorHandler(client, lastSocketCreateTime) {
