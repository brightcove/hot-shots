...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  if (typeof this.globalTags === 'object') {
    this.globalTagsMemo = helpers.overrideTagsToString(this.globalTags, [], this.telegraf, this.tagSeparator);
  }
  else {
    this.globalTagsMemo = '';
  }
...
Client.prototype.send = function (message, tags, callback) {
  const tagsString = getTagString(this.globalTags, this.globalTagsMemo, tags, this.telegraf, this.tagSeparator);
...
function getTagString(globalTags, globalTagsMemo, tags, telegraf, tagSeparator) {
  if (!tags) {
  ...
  if (isTagsArray || Array.isArray(globalTags)) {
    return helpers.overrideTagsToString(globalTags, tags, telegraf, tagSeparator);
  }
  else {
    return helpers.overrideTags2(globalTags, globalTagsMemo, tags, telegraf, tagSeparator);
  }
}
...
const ChildClient = function (parent, options) {
  options = options || {};
  const yy = typeof options.globalTags === 'object' ? helpers.overrideTags(parent.globalTags, options.globalTags, parent.telegraf) : parent.globalTags;
  Client.call(this, {
    isChild     : true,
