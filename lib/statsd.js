...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  const createSocket = function createSocket(instance, args) {
    ...
    if (args.protocol === TCP) {
      ...
      if (! unixDgram) {
        errMessage = 'The library unix_dgram, needed for UDS, is not available on your OS or version of Node.';
      }
      else if (! args.path) {
        errMessage = 'A path must be given to use UDS.';
      }
      else {
        try {
          ...
          errMessage = `Could not establish UDS connection to ${args.path}: ${err}`;
        }
      }
    } else {
      try {
...
Client.prototype._close = function (callback) {
  ...
  try {
    if (this.protocol === TCP) {
      ...
      this.socket.close();
      // this close is synchronous so we callback right away
      callback();
    }
    else {
      this.socket.close();
