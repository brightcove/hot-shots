...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  if (typeof this.globalTags === 'object') {
    this.globalTagsMemo = helpers.overrideTags(this.globalTags, [], this.telegraf, this.tagSeparator);
  }
  else {
    this.globalTagsMemo = '';
  }
...
function getTagString(globalTags, globalTagsMemo, tags, telegraf, tagSeparator) {
  ...
  if (isTagsArray || Array.isArray(globalTags)) {
    return helpers.overrideTags(globalTags, tags, telegraf, tagSeparator);
  }
  else {
